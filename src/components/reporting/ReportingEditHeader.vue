    ...
    <smart-header>
        ...
        <el-row type="type" align="middle" style="height: 100%;">
            ...
            <el-col :span="8" style="display: flex;align-items: center;height: 100%;">
                <img src="../../assets/image/logo-white.png" style="height:38px;margin:0 20px;" draggable="false"/>
                <h2 style="font-weight: 600;font-size: 20px;color:white;">报告编辑</h2>
                <slot name="left"></slot>
            </el-col>
            <!--中间-->
            <el-col :span="8" style="display: flex;align-items: center;justify-content:center;height: 100%;">
                <!--图表项-->
                <div class="edit-header-wrapper" v-for="(dirItem,dirIndex) in $options.dataComponents" :key="dirIndex">
                    ...
                    <smart-select class="edit-header-item" style="height: 350px;z-index: 1000;">
                        <template v-for="(item,index) in dirItem.components">
                            ...
                            <template v-if="!!item.components">
                                <smart-select-item :label="item.title" :key="index">
                                    <el-row :gutter="5">
                                        <el-col :span="8" style="margin-bottom: 5px;" v-for="(fileItem,fileIndex) in item.components" :key="fileIndex">
                                            <display-charts :image="fileItem.image" @click.native="handleSelectWidget(fileItem.data)">
                                                {{fileItem.data.title}}
                                            </display-charts>
                                        </el-col>
                                    </el-row>
                            ...
                            <template v-else>
                                <smart-select-item :label="item.data.title" :key="index">
                                    <el-row :gutter="5">
                                        <el-col :span="8" style="margin-bottom: 5px;">
                                            <display-charts :image="item.image" @click.native="handleSelectWidget(item.data)">
                                                {{item.data.title}}
                                            </display-charts>
                                        </el-col>
                                    </el-row>
            ...
            <!--右侧-->
            <el-col :span="8" style="display: flex;align-items: center;;height: 100%;justify-content: flex-end;">
