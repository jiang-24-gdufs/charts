<template>
    <div>
        ...
        <div class="data-config-header">
            <img src="../../assets/icon/data-source.svg" alt="" draggable="false" style="margin-right: 5px;">
            数据源
        </div>
        <empty-data-model style="height: calc(100% - 51px);" v-if="!showDataModel">
        ...
        <div class="data-config-title" v-else>
            <el-select v-model="dataSetName" @change="handleDataSetChange" placeholder="请选择数据集" size="mini"
                       style="margin: 10px 0;width: 95%;">
                <el-option v-for="(item,index) in dataSetList"
                           :key="index"
                           :label="item.name"
                           :value="item">
                </el-option>
            </el-select>
            <data-source-choice  v-if="dataSetName !== null" :data-set-item="dataSet" style="height:calc(100% - 60px);"></data-source-choice>
        </div>
    </div>
</template>
